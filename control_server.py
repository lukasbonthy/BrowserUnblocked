...
def dash(u):
 ...
 return page('Dashboard','<section class=card><h1>Welcome, %s</h1><p>Each account gets its own KasmVNC desktop. Capacity: %s/%s active sessions.</p><p><a class=btn href="%s">Open current session</a> <a class=btn href=/release>End session</a></p></section><div class=grid style="margin-top:14px">%s</div>'%(html.escape(u['name']),used,MAX_SESSIONS,openlink,cards),u)
...
if __name__=='__main__': init(); print('BrowserUnblocked control server listening on http://%s:%s'%(HOST,PORT),flush=True); print('BrowserUnblocked multi-session storage: '+STORE,flush=True); print('BrowserUnblocked capacity MAX_ACTIVE_SESSIONS=%s'%MAX_SESSIONS,flush=True); ThreadingHTTPServer((HOST,PORT),H).serve_forever()
